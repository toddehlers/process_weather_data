...
("=============================================================================")
("          EARTHSHAPE CHILE WEATHER STATIONS DATA MANAGEMENT (V9, Sep 2020)   ")
("=============================================================================")
"""
...
Otherwise print the missing packages for the user
============================================================================="""
msng_pkg=[]
try:
    import sys
...
def defwhat():
    ...
    if what == "0": # switch the menu from short to extended
        ...
        what = input("________________________________________"
                     ...
                     "   -Extract and display all the information of the combined file \n"                 
                     ...
                     "\n5: Monthly Temperature and Precipitation Excel file (mean & std) \n"
                     "***After Execution: \n"   
...
def defuiopen(title_text,fp):
    if len(fp) == 0:
        ...
        d_type,station_name,fn,t_start,t_end = [],[],[],[],[]
#..............................................................................
...
# make a sound when opens a file dialog    
        if platform == "darwin" or platform == "linux": # in mac
            print('\a') 
        elif platform == "win32": # in windows
            winsound.Beep(frequency, duration)
#..............................................................................
# open a file dialog and extract the path of the selected file        
...
#..............................................................................
    t_start = data.iloc[0, 0] # date and time of the first recorded data
    t_end = data.iloc[len(data) - 1, 0] # date and time of the last recorded data
    nan_ind = pd.isnull(data).any(1).to_numpy().nonzero()[0] # find the index of rows with at least one missing (nan) value
    time = data.iloc[:,0] # time vector
    t_delta_time = []
...
# Convert units
    ...
    return data,pi,si
"""=============================================================================
FUNCTION (Check and find the rows with missing data):
============================================================================="""
...
"""=============================================================================
FUNCTION (Maintenances and Filtering):
...
Conver the unit of "EC_Avg" to mS/m for the data before May 2017
============================================================================="""
def defEC_unit(data,station_name,si,pi):
    ...
    # check the header of the file to see if the unit was corrected
    if si.iloc[0,14] != "crct_unit": # if not corrected then correct it
        B = data.iloc[:,0] < ind_c
    ...
    else: # if it was already corrected check the time of the last correction and apply correction for the latest data
...
#..............................................................................    
    while True:
        ...
        fp,si,pi,data,_,_,_,_,_ = defuiopen("Select a *.CSV file",[]) # select a new file
...
"""=============================================================================
FUNCTION (Linear trend / Moving average:
...
def deftrend(LPF_m,data,datplt,ind_p):
    ...
    dta = data.iloc[:,ind_p].to_numpy()     
    dta[:,(dta == -9999).all()] = np.nan
    ...
    if ("3" in datplt) or ("4" in datplt):
        ...
        for i in range(int((LPF_m-1)/2)-1,int(nt-((LPF_m-1)/2))): #loop through time steps (leave margin depending on LPF setup)
            for v in range(nv): #count missing values for variables separately
                ...
                nan_ind = list(pd.isnull(dta[:,[v]]).any(1).to_numpy().nonzero()[0])
                datLPF[nan_ind,v] = np.nan
    """Linear trend & de-trend"""            
    if ("2" in datplt) or ("5" in datplt) or ("6" in datplt):
        ...
        for v in range(nv):
            ...
            y = dta[:,v]
            m,b,_,_,_ = stats.linregress(x[ind_not_nan],y[ind_not_nan]) # gettinh linear regression
            regLine[:,v] = m*x+b # linear regression line          
            datDeTr[:,v] = y-(m*x) # remove the linear trend from the data
        ...
        if ("1" in sctn) or ("1" in sctn_end):
            while True:
                maintfilt = input("________________________________________ FILTER (MAINTENANCE & ANOMALIES) ???"
                             "\n1: Fiter the data (I have the maintenance file)"
                             "\n2: Filter the data (I do not have the maintenance file)"
...
#                table_vals = [y_MAN,y_MED,y_SDV,y_PER]
            ...
            if sv_nsv == "1" or sv_nsv == "3": # to show the plot
                ...
                elif platform == "darwin" or platform == "linux": # in mac
...
# Import the second file                     
                    while True:
                        print("________________________________________ IMPORT A '" + d_type1.upper() + "' FILE FROM '" + station_name1.upper() + "' STATION")
...
"""=============================================================================
FUNCTION (Sat and Loc filling visualization and REPORT):
Visualize how the local file can fill the satellite file
REPORT number of gaps in satellite file
...
#    ax.xaxis.set_major_locator(locator)
    ...
    return new_local,j
"""=============================================================================
FUNCTION (Fill the gaps):
Starts to fill the gaps in the satellite file based on the local data
============================================================================="""        
...
# import an associated local file                            
                            print("________________________________________ IMPORT A LOCAL FILE FROM '" + station_name_s.upper() + "' STATION") 
...
# Satellite file is started later than the local data
                                            ...
                                            nan_ind_s = pd.isnull(data_s).any(1).to_numpy().nonzero()[0] # find the index of rows with at least one missing (nan) value
#..............................................................................
...
**********************************************************************************************************"""
"""=============================================================================
FUNCTION: (Monthly mean&std of temperatute and pressure)
...
#..............................................................................                        
                    ...
                    for m in range(1,13):
                        ...
                        for y in range(fy,ly+1):                            
                            ...
                            nan_ind_T = pd.isnull(temp_T).any(1).to_numpy().nonzero()[0]
                            nan_ind_P = pd.isnull(temp_P).any(1).to_numpy().nonzero()[0]
